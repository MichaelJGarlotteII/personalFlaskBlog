...
After the initial launch, James in his free time decided to develop an upvote/downvote feature for the
blog posts that caused a few trials that we were able to overcome, particularly ones involving GitHub
and versioning. When I pulled his files, I lost a few important lines of code that resulted in the feature
not functioning. Once we solved that problem it was on to Dockerizing the updated site, something my Windows
machine decided to no longer let me do. I then moved all my Dockerization over to a Ubuntu VM on VirtualBox
as a patch solution until I figure out why my host OS won't let Docker function correctly.
...
flaskBlog directory--
      ...
      static directory--
            ...
            main.css: This is the CSS file created by the author of the YouTube tutorial from above. No changes have been made
                      to this as of yet.

            profile_pics directory--
                  --This directory contains all profile pics that get saved to the site by users. It will be static on GitHub since
                    GitHub doesn't receive new data from the server.
